...
/**
 * Client component fetching and rendering a single role by slug.
 */
export default function ClientRolePage({ slug }: { slug: string }) {
  // Fetch the role by ID. `undefined` means loading, `null` or falsy means not found.
  const role = useQuery(api.roles.getRoleById, { id: slug });
  if (role === undefined) {
    // Still loading query; render nothing or a loading state
    return null;
  }
  if (role === null) {
    // Role not found; show message
    return (
            ...
            <div className="flex-1">
              ...
              {role.alternateNames.length > 0 && (
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {role.alternateNames.map((name: string, idx: number) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <CardContent className="space-y-6">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {role.technicalSkills.map((skill: string, idx: number) => (
                      <Badge
                        key={idx}
                        variant="secondary"
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {role.softSkills.map((skill: string, idx: number) => (
                      <Badge
                        key={idx}
                        variant="secondary"
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-6">
                  {role.careerLadder.map((level: any, idx: number) => (
                    <div
                      key={idx}
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        ...
                        {idx < role.careerLadder.length - 1 && (
                          <Progress
                            value={((idx + 1) / role.careerLadder.length) * 100}
                            className="mt-2"
                          />
                        )}
                      </div>
          ...
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  {role.industry.map((ind: string, idx: number) => (
                    <div
                      key={idx}
            ...
            {role.hiringCompanies && role.hiringCompanies.length > 0 && (
              <Card>
                ...
                <CardContent className="space-y-4">
                  {role.hiringCompanies.map((companyGroup: any, idx: number) => (
                    <div key={idx} className="space-y-2">
                      <h4 className="text-sm font-semibold text-primary">
                        {companyGroup.category}
                      </h4>
                      <div className="flex flex-wrap gap-1.5">
                        {companyGroup.companies.map((company: string, companyIdx: number) => (
